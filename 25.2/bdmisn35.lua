...
local defendComm = mission.Objective:define("defendComm"):setListeners({
    init = function(self)
        ...
        self.walker = GetHandle("george");
    end,
    start = function(self)
        self.wdead = false;
        AddObjective("bdmisn2501.otf","white");
        AddObjective("bdmisn2502.otf","white");
        globals.attackOtf = "bdmisn2502.otf";
    end,
    update = function(self,dtime)
        if(not IsAlive(self.comm)) then
            self:fail();
        end
        if(not (self.wdead or IsValid(self.walker)) ) then
            self.wdead = true;
            AudioMessage("bdmisn2502.wav");
            RemoveObjective("bdmisn2502.otf");
            globals.attackOtf = "bdmisn2503.otf";
    ...
    fail = function(self)
    	UpdateObjective("bdmisn2501.otf","red");
...
local destorySovietComm = mission.Objective:define("destorySovietComm"):setListeners({
    ...
    update = function(self,dtime)
        ...
        if( (not IsAlive(self.scomm) ) and not (self.scc)) then
            UpdateObjective(globals.attackOtf,"green");
            self.scc = true;
        end
        if(self.scc) then
...
local baseDestroyCin = mission.Objective:define("baseDestroyCin"):setListeners({
    ...
    update = function(self,dtime)
        ...
        elseif(self.waitleft <= 10) then
            ...
            for v in ObjectsInRange(500,GetHandle("lpad")) do
                if(GetPlayerHandle() ~= v) then
                    if((not IsBuilding(v)) and (GetTeamNum(v) == 1) ) then
                        Damage(v,GetMaxHealth(v)/7 * dtime);
                    end
                end
            end
        end
        ...
        if(self.cam) then
            ...
            elseif(self.camstage == 4) then
                t = GetHandle("recycler");
                p = "25cin_pan2";
            end

            if(p) then
                CameraPath(p,1000,100,t);
            else
    ...
    success = function(self)
        ...
        for v in ObjectsInRange(500,GetHandle("lpad")) do
            if(GetPlayerHandle() ~= v) then 
                if(not IsBuilding(v)) then
                    Damage(v,100000);
                end
            end
        end
        --Make sure to destroy everything within 500m
...
local pickupSurvivors = mission.Objective:define("pickupSurvivors"):setListeners({
    ...
    start = function(self)
        --Play audio
        RemoveObjective("bdmisn2501.otf");
        AudioMessage("bdmisn2601.wav");
        ...
        SetObjectiveName(navs[1],"Black Dog Outpost");
        ...
        if(not self.pilots) then
            if(IsWithin(self.apcs[1],self.nav,200) or 
               ...
               IsWithin(GetPlayerHandle(),self.nav,200)) then
                self.pilots = {};
                for i=1,6 do
                    local p = BuildObject("bspilo", 1, "spawn_pilo" .. i)
                    table.insert(self.pilots,p);
                    SetIndependence(p,0);
                end
            end
        else
    ...
    fail = function(self,v)
        UpdateObjective("bdmisn2601.otf","red");
        if v == 1 then 
            FailMission(GetTime()+5.0,"bdmisn26l1.des"); 
        else
...
miss26setup = function()
    ...
    UpdateObjective("bdmisn2501.otf","red");
    RemoveObjective(globals.attackOtf);
    spawnAtPath("proxmine",2,"spawn_prox");
    ...
    for i, v in pairs(apcs) do
        ...
        SetObjectiveOn(v);
        Goto(v,"26apc_meatup",0);
    end
    for i, v in pairs(spawnAtPath("bvtank",1,"26spawn_tank")) do
        Goto(v,"26apc_meatup",0);
    end
    Goto(spawnAtPath("bvhraz",1,"26spawn_bomber")[1],"26bomber_rev",0);
    apcMeatup:start();
end
